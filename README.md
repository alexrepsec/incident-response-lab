...
> **Tags:** `Incident Response` `SIEM` `Linux Security` `Bash` `Python` `JSON` `auditd` `Fail2ban` `Wazuh` `VMware`

A fully functional home lab simulating a real-world attacker vs. defender scenario built on VMware with Ubuntu Server and Kali Linux. An automated detection engine monitors the system in real time, and when an attack is detected, it triggers a multi-phase response: evidence collection, IP containment, and auto-generated incident reports — all without human intervention.
...
| `Python` (via auditd) | Log parsing and audit event processing |
...
*Part of my cybersecurity portfolio — [alexrepsec](https://github.com/alexrepsec)*
